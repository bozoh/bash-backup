...
then
	if [ $PG_VERSION -le 8 ]
	then
		echo "su - $USUARIO_POSTGRES -c \"pg_dump $BANCO -n$ESQUEMA -d -E$ENCONDING -Fc -U$USUARIO_POSTGRES -h$HOST -f$ARQUIVO\""
	else
		echo "su - $USUARIO_POSTGRES -c \"pg_dump $BANCO -n$ESQUEMA --inserts -E$ENCONDING -Fc -U$USUARIO_POSTGRES -h$HOST -f$ARQUIVO\""
	fi
else
	#dependendo da versão do postgres, não aceita a opção -E no dump
	if [ $PG_VERSION -le 8 ]
	then
		su - $USUARIO_POSTGRES -c "pg_dump $BANCO -n$ESQUEMA -d -E$ENCONDING -Fc -U$USUARIO_POSTGRES -h$HOST -f$ARQUIVO"
	else
		su - $USUARIO_POSTGRES -c "pg_dump $BANCO -n$ESQUEMA --inserts -E$ENCONDING -Fc -U$USUARIO_POSTGRES -h$HOST -f$ARQUIVO"
	fi
